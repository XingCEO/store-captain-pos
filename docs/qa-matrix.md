...
| POST /api/v1/users | ✅ | ✅ USER_INVALID | n/a | ✅ ROLE_GRANT_FORBIDDEN | 電郵唯一性 + 角色層級檢驗 |
| PATCH /api/v1/users/:id | ✅ | ✅ USER_INVALID | n/a | ✅ ROLE_GRANT_FORBIDDEN | 非 MANAGER 不可修改 |
...
| POST /api/v1/order-sources/manual | ✅ | ✅ ORDER_ITEM_INVALID | ✅ SOURCE_DUPLICATE | ✅ PERMISSION_DENIED (CASHIER+) | 外部參考去重 |
| POST /api/v1/channels/qr/orders | ✅ | ✅ ORDER_ITEM_INVALID | ✅ CATALOG_IDEMPOTENCY_CONFLICT | ✅ PERMISSION_DENIED (CASHIER+) | QR 渠道建立 |
| POST /api/v1/channels/line/orders | ✅ | ✅ CHANNEL_AUTH_FAILED | ✅ CATALOG_IDEMPOTENCY_CONFLICT | ✅ PERMISSION_DENIED (CASHIER+) | LINE token 驗證 |
| PATCH /api/v1/channels/orders/:id/status | ✅ | ✅ SOURCE_ITEM_CLOSED | ✅ OUT_OF_STOCK | ✅ PERMISSION_DENIED (MANAGER+) | 渠道訂單狀態轉移 |
...
| GET /api/v1/inventory/levels | ✅ | n/a | n/a | ✅ PERMISSION_DENIED (MANAGER+) | 庫存水位列表 |
| POST /api/v1/inventory/adjustments | ✅ | ✅ INVENTORY_ADJUSTMENT_INVALID | ✅ OUT_OF_STOCK | ✅ PERMISSION_DENIED (MANAGER+) | 手動調整 + 審計 |
...
| GET /api/v1/reports/top-products | ✅ | ✅ DATE_RANGE_INVALID | n/a | ✅ PERMISSION_DENIED (MANAGER+) | 熱銷商品排行 |
| POST /api/v1/reports/exports | ✅ | ✅ EXPORT_RANGE_INVALID | ✅ EXPORT_RANGE_TOO_LARGE | ✅ PERMISSION_DENIED (MANAGER+) | 報表生成 + 92 天限制 |
| GET /api/v1/reports/exports/:id | ✅ | n/a | n/a | ✅ PERMISSION_DENIED (MANAGER+) | 查詢匯出狀態 |
| GET /api/v1/reports/exports/:id/download | ✅ | ✅ FILE_EXPIRED | n/a | ✅ TENANT_NOT_AUTHORIZED | Token 驗證 + 過期檢查 |
...
| POST /api/v1/customers | ✅ | ✅ CUSTOMER_PHONE_INVALID | n/a | ✅ PERMISSION_DENIED (CASHIER+) | 台灣手機驗證 |
| GET /api/v1/customers/search | ✅ | n/a | n/a | ✅ PERMISSION_DENIED (CASHIER+) | 按電話搜尋 |
| GET /api/v1/coupons | ✅ | n/a | n/a | ✅ PERMISSION_DENIED (CASHIER+) | 活躍折扣券列表 |
| POST /api/v1/coupons/redeem | ✅ | ✅ COUPON_NOT_APPLICABLE | ✅ COUPON_NOT_APPLICABLE | ✅ PERMISSION_DENIED (CASHIER+) | 最低消費檢查 |
| POST /api/v1/customers/points/adjust | ✅ | ✅ POINTS_INVALID | n/a | ✅ PERMISSION_DENIED (MANAGER+) | 點數調整 + 歷史記錄 |
| GET /api/v1/ai/daily-brief | ✅ | n/a | n/a | ✅ PERMISSION_DENIED (MANAGER+) | AI 日報摘要 |
| POST /api/v1/telemetry/heartbeat | ✅ | ✅ SYNC_STALE | n/a | ✅ PERMISSION_DENIED (CASHIER+) | 終端心跳 + 狀態評估 |
| GET /api/v1/telemetry/dashboard | ✅ | ✅ DEVICE_MISMATCH | n/a | ✅ PERMISSION_DENIED (MANAGER+) | 終端狀態儀表板 |
